...
public class DefaultBackgroundOrderQueue : IBackgroundOrderQueue
{
    ...
    public ValueTask QueueBackgroundOrderStatusAsync(Func<CancellationToken, ValueTask<Order>> workItem)
        => _queue.Writer.WriteAsync(workItem);

    public ValueTask<Func<CancellationToken, ValueTask<Order>>> DequeueAsync(CancellationToken cancellationToken)
